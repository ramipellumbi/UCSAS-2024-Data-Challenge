...
results <- results <- lapply(c("m", "w"), function(gender_t) {
  ...
  team_results <- foreach (team_sim = 1:TEAM_SIM, .export = c("alternates", "usa", "non_usa_team", "models"), .combine = 'c') %dorng% {
    ...
    save_results(list(team_sim_results = sample_results, usa_team = usa_options %>% dplyr::select(name, run, gender)), results_filename)
